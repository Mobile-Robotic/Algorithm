#include <stdio.h>
#include <math.h>

#define X 3
#define Y 5
#define N X*Y
#define MAX 580
#define M 99

int front, rear, parent[MAX], step_cnt, Path_cnt;
char queue[MAX][2], temp_IP[N], stack[60][2], pose[2];

#define CASE 1

#if CASE==1
char IP[N]={
	0,1,0,2,0,
	0,M,3,M,1,
	3,0,0,2,0
};
char SS[N]={
	0,3,0,1,2,
	0,M,0,M,0,
	0,2,3,0,1
};
#elif CASE==2
char IP[N]={
	1,0,2,3,0,
	0,M,2,M,0,
	3,0,1,0,0
};
char SS[N]={
	0,3,0,1,2,
	0,M,0,M,0,
	0,2,3,0,1
};
#elif CASE==3
char IP[N]={
	0,0,2,1,3,
	0,0,M,M,0,
	0,0,2,1,3
};
char SS[N]={
	0,0,3,2,1,
	0,0,M,M,0,
	0,0,3,2,1
};
#elif CASE==4
char IP[N]={
	1,0,2,3,0,
	0,M,2,M,0,
	3,0,1,0,0
};
char SS[N]={
	0,0,0,3,0,
	0,M,0,M,0,
	1,2,3,2,1
};
#elif CASE==5
char IP[N]={
	1,1,2,0,0,
	0,M,2,M,0,
	3,0,0,0,3
};
char SS[N]={
	0,2,2,0,0,
	0,M,0,M,0,
	1,3,3,0,1
};
#endif

char tIP[5], tSS[5];
char SNode[2][5]={
	{0,4,1,3,2},
	{10,14,11,13,12},
};
char TargetNode, TargetColor, TargetStep;
char SortDir;

char enqueue(char arr[]){
	if(rear<MAX){
		queue[rear][0]=arr[0];
		queue[rear++][1]=arr[1];
		return 1;
	}
	return 0;
}
char *dequeue(){
	if(front<rear) return queue[front++];
	return NULL;
}
void push(char x, char y){
	stack[step_cnt][0]=x;
	stack[step_cnt++][1]=y;
}
void swap(char *a, char *b){
	char tmp=*a;
	*a=*b;
	*b=tmp;
}
void copy(char a[], char b[]){
	for(int i=0;i<N;i++) a[i]=b[i];
}
char Change(char value, char mode){
	if(mode==0) return value/Y;
	if(mode==1) return value%Y;
	return 0;
}
void Pose_init(char d){
	if(d==0) pose[0]--;
	if(d==1) pose[1]++;
	if(d==2) pose[0]++;
	if(d==3) pose[1]--;
}
char Find_Node(char x, char y){
	return x*Y+y;
}
char safe(char x, char y){
	return 0<=x && x<X && 0<=y && y<Y;
}
void Link(int a, char mode){
	if(a>0) Link(parent[a],mode);
	else{
		copy(temp_IP,IP);
		return;
	}
	if(mode) push(queue[a][0],queue[a][1]);
	swap(&temp_IP[queue[a][0]],&temp_IP[queue[a][1]]);
}
void Path_S(int i){
	if(i<step_cnt && stack[i][1]==stack[i+1][0]){
		Path_cnt++;
		Path_S(i+1);
	}
}
char same(char a[], char b[]){
	for(int i=0;i<N;i++) if(a[i]!=b[i] && b[i]) return 0;
	return 1;
}
char s_same(char a[]){
	for(int i=0;i<rear;i++){
		Link(i,0);
		if(same(temp_IP,a)) return 1;
	}
	return 0;
}

// Debug
void draw(char a[]){
	for(int i=0;i<N;i++){
		printf("%4d",a[i]);
		if(Change(i,1)==Y-1) printf("\n");
	}
	printf("\n");
}

void change_to_2x5(char mode, char dir){
	if(mode==0) {
		for(int i=0; i<5; i++) {
			tIP[i]=IP[i+!dir*Y*2];
			tSS[i]=SS[i+!dir*Y*2];
			IP[i+!dir*Y*2]=SS[i+!dir*Y*2]=M;
		}
	}
	else {
		for(int i=0; i<5; i++) {
			IP[i+!dir*Y*2]=tIP[i];
			SS[i+!dir*Y*2]=tSS[i];
		}
	}
}
char BFS_is_partsame(char a[], char mode){
	if(mode==1){
		for(int i=0;i<N;i++){
			if(a[i]==TargetColor && abs(Change(TargetNode,0)-Change(i,0))+abs(Change(TargetNode,1)-Change(i,1))<TargetStep){
				copy(IP,a);
				return 1;
			}
		}
	}
	else if(mode==2){
		if(a[TargetNode]==SS[TargetNode]){
			a[TargetNode]=SS[TargetNode]=M;
			copy(IP,a);
			return 1;
		}
	}
	else if(mode==3){
		for(int i=0;i<5;i++) if(a[SNode[SortDir][i]] && a[SNode[SortDir][i]]!=M) return 0;
		copy(IP,a);
		return 1;
	}
	else if(mode==4){
		if(same(a,SS)){
			copy(IP,a);
			return 1;
		}
	}
	return 0;
}
char BFS(char mode){
	char a[N], b[N], *p;
	char point[2]={0,};
	
	front=rear=0;
	enqueue(point);
	while(1){
		p=dequeue();
		if(p==NULL) return 0;
		Link(front-1,0);
		copy(a,temp_IP);
		if(BFS_is_partsame(a,mode)) return 1;
		for(int i=0,j=0;i<N;i++){
			if(a[i] && a[i]!=M){
				for(int D=0;D<4;D++){
					pose[0]=Change(i,0);
					pose[1]=Change(i,1);
					Pose_init(D);
					j=Find_Node(pose[0],pose[1]);
					if(safe(pose[0],pose[1]) && a[j]==0){
						copy(b,a);
						swap(&b[i],&b[j]);
						if(s_same(b)==0){
							parent[rear]=front-1;
							point[0]=i;
							point[1]=j;
							if(!enqueue(point)) return 0;
							if(BFS_is_partsame(b,mode)) return 1;
						}
					}
				}
			}
		}
	}
	return 0;
}
char BFS_run(char mode){
	char state=BFS(mode);
	if(state==1) Link(rear-1,1);
	printf("mode=%d, state=%d, rear=%d\n",mode,state,rear);
	return state;
}
void solve(){
	char cnt=0;
	for(int i=0;i<5;i++) if(SS[i]) cnt++;
	if(cnt<3) SortDir=1;
	for(int i=0;i<5;i++){
		TargetNode=SNode[SortDir][i];
		TargetColor=SS[TargetNode];
		if(TargetColor && TargetColor!=M){
			for(int j=0;j<3;j++){
				TargetStep=3-j;
				BFS_run(1);
			}
			BFS_run(2);
		}
	}
	BFS_run(3);
	SortDir=!SortDir;
	change_to_2x5(0,SortDir);
	BFS_run(4);
	change_to_2x5(1,SortDir);
}
int main(){
	char fIP[N], fSS[N];
	copy(fIP,IP);
	copy(fSS,SS);
	solve();
	for(int i=0;i<step_cnt;i++){
		Path_cnt=0;
		Path_S(i);
		printf("from %d to %d\n",stack[i][0],stack[i+Path_cnt][1]);
		swap(&fIP[stack[i][0]],&fIP[stack[i+Path_cnt][1]]);
		draw(fIP);
		i+=Path_cnt;
	}
	printf("step_cnt=%d\n",step_cnt);
	if(same(fIP,fSS)) printf("Success!!");
	else printf("fail");
}
